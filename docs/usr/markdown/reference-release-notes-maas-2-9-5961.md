...
See the [VM hosting](https://canonical.com/maas/docs/about-virtual-machines) page for more details, and be sure to use the menu at the top of that page to select your desired build method and interface, so that you'll see the most relevant instructions.
...
You can see the results of the investigation in the MAAS documentation.
...
A number of MAAS issues have actually been issues with an older version of Curtin.  MAAS now includes Curtin 20.2, which fixes many of these issues.
...
- maas_auto_ipmi_k_g_bmc_key - she IPMI K_g pre-shared encryption key to be set when adding the MAAS IPMI user.  Note not all IPMI BMCs support setting the k_g key, if MAAS is unable to set the key commissioning will fail.  Default comes from the global configuration setting.  If an IPMI K_g key is set but the key is rejected by the BMC MAAS will automatically retry without the K_g key.  This works around an edge case where some BMCs will allow you to set an K_g key but don’t allow it to be used.
...
See the [power management page](https://canonical.com/maas/docs/reference-power-drivers) for details.
...
See the [How to use logging](https://canonical.com/maas/docs/how-to-use-logging) documentation for more details on these changes.
...
- The Proxmox driver [uses a hard-coded port that cannot be customized](https://bugs.launchpad.net/maas/+bug/1914165).  This port is now customisable in 2.9.2.
